...
if [ -n "${USE_VALGRIND}" ]; then
    ...
    if [ -z "$(cat "$(dirname "$0")/report.txt")" ]; then
        if ! "$(dirname "$0")/comparator.sh" "$(dirname "$0")/temp.txt" "${args_arr[0]/"in"/"out"}"; then
            exit $ERROR_ONLY_MEM_OK
        fi
    elif "$(dirname "$0")/comparator.sh" "$(dirname "$0")/temp.txt" "${args_arr[0]/"in"/"out"}"; then
        exit $ERROR_ONLY_TEST_OK
    else
        exit $ERROR_NOTHING_OK
    fi
else
    "$(dirname "$0")/../../app.exe" $args
