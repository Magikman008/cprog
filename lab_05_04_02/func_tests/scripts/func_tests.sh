...
pos_tests=$(find "$(dirname "$0")/../data" -mindepth 1 -maxdepth 1 | sort -t '\0' -n | grep -E '+pos_[0-9][0-9]_in.txt$')
pos_passed_ok=0
pos_passed_mem=0
pos_passed=0
pos_count=0
...
if ! [ $pos_count -eq 0 ]; then
    ...
    echo "No positive tests were found"
fi
echo

neg_tests=$(find "$(dirname "$0")/../data" -mindepth 1 -maxdepth 1| sort -t '\0' -n | grep -E '+neg_[0-9][0-9]_in.txt$')
neg_passed_ok=0
neg_passed_mem=0
neg_passed=0
neg_count=0
...
if ! [ $neg_count -eq 0 ]; then
    ...
    echo "No negative tests were found"
fi
